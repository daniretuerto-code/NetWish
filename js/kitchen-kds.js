...
window.openKitchenOrdersModal = async function() {
    ...
    const page = document.getElementById('view-kitchen-kds');
    if (!page) return;
...
window.closeKitchenOrdersModal = function() {
    const page = document.getElementById('view-kitchen-kds');
    if (page) {
...
window.renderKitchenOrdersList = async function() {
    const container = document.getElementById('kitchenDedicatedContainer');
    if (!container || !currentBusiness) return;

    container.innerHTML = `
        <div class="py-12 text-center space-y-2">
            <i data-lucide="loader-2" class="w-6 h-6 mx-auto animate-spin text-neutral-400"></i>
            <p class="text-xs text-neutral-400">Cargando comandas en cocina...</p>
        </div>
    `;
    ...
    if (tableOrders.length === 0) {
        container.innerHTML = `
            <div class="py-20 text-center space-y-3">
                <div class="w-16 h-16 rounded-3xl bg-neutral-50 border border-neutral-200/60 flex items-center justify-center mx-auto text-neutral-400 shadow-inner">
                    <i data-lucide="chef-hat" class="w-8 h-8"></i>
                </div>
                <h3 class="text-sm font-bold text-black">No hay comandas pendientes</h3>
                <p class="text-xs text-neutral-400 px-6">Las comandas que envíen los comensales desde las mesas aparecerán aquí al instante.</p>
            </div>
        `;
    } else {
